function plot_random_trajectory_on_real_map(agentFile)
...
for step = 1:cfg.maxSteps
    ...
    % Build observation dynamically (dimension-safe)
    obs = zeros(obsDim,1);
    ...
    % Check action type (continuous vs discrete)
...
%% ================= ADVANCED SMOOTH VISUALIZATION =================

% Remove zero rows (if early stop)
validIdx = latPath ~= 0;
latPath = latPath(validIdx);
lonPath = lonPath(validIdx);

% Step 1: Light downsampling (reduces jitter density)
latPath = latPath(1:2:end);
lonPath = lonPath(1:2:end);

% Step 2: Strong smoothing
latSmooth = smoothdata(latPath,'gaussian',11);
lonSmooth = smoothdata(lonPath,'gaussian',11);

% Step 3: Spline interpolation for curved look
t  = 1:length(latSmooth);
tt = linspace(1,length(latSmooth),length(latSmooth)*5);

latInterp = spline(t,latSmooth,tt);
lonInterp = spline(t,lonSmooth,tt);

%% ================= GEO MAP VISUALIZATION =================
figure('Name','DRL-NAV Trajectory (Professional View)');
gx = geoaxes;
hold(gx,'on');
...
% UAV Path (Interpolated smooth curve)
geoplot(gx,latInterp,lonInterp,'b','LineWidth',3);
