function drl_nav_train_ppo_3phase()
% DRL-NAV ALL-IN-ONE:
% 1) Environment (rlFunctionEnv)
% 2) PPO training (3-phase schedule)
% 3) Run trained policy and plot trajectory
...
actor = rlDiscreteCategoricalActor( ...
    actorNet, obsInfo, actInfo, ObservationInputNames="obs");
...
% IMPORTANT: we will modify these per phase:
% agentOpts.EntropyLossWeight
...
trainOpts.MaxEpisodes = 600;
stats1 = train(agent, env, trainOpts);
save("drl_nav_agent_phase1.mat","agent","stats1");

% =========================
% PHASE 2: Stabilization
% Episodes: 601..1200 (additional 600)
% =========================
...
save("drl_nav_agent_phase2.mat","agent","stats1","stats2");

% =========================
% PHASE 3: Precision Optimization
% Episodes: 1201..2000 (additional 800)
% =========================
...
save("drl_nav_agent_final.mat","agent","stats1","stats2","stats3");

% ---------- Run & plot one rollout ----------
drl_nav_run_trained_policy_map(agent, 350);
...
eastDistKm = drl_nav_lon_dist_km(lat2, cfg.lonMax - lon2);
eastN = min(max((cfg.eastBufferKm - eastDistKm) / cfg.eastBufferKm, 0), 1);
...
% ================= REWARD (stable + meaningful) =================
rewardGoalProgress   = 400 * progress;
rewardDistanceShape  = -0.5 * newDist;
...
% =====================================================================
% RUN TRAINED POLICY + PLOT
% =====================================================================
function drl_nav_run_trained_policy_map(agent, maxSteps)
if nargin < 2, maxSteps = 350; end
...
for t = 1:maxSteps
    ...
    [obs, r, done, logged] = feval(getStepFcn(env), act, logged); %#ok<ASGLU>
    if done
        latPath = latPath(1:t);
        lonPath = lonPath(1:t);
        break;
...
figure('Name','DRL-NAV UAV Trajectory (Trained PPO Policy)');
